...
public class OgStepGenerator {
    ...
    static private int getRandomHold() {
        ...
        {
            pickHold = rand.nextInt(hc);
        }
        else
        {
            pickHold = lastHoldIndex++ % hc;
        }
        for(int i=0;i<4;i++) {
            if( holding[i] > 0f ) {
    ...
    public static String GenerateNotes(int stepGranularity, int skipChance,
        ...
        for(float t = timeOffset; t <= totalTime; t += timeGranularity) {
            ...
            if( t > 0f ) {
                ...
                if( AutoStepper.RANDOMIZED )
                {
                    skipRoll = rand.nextInt(skipChance);
                }
                else
                {
                    skipRoll = lastSkipRoll++ % skipChance;
                }
                if( timeIndex % 2 == 1 &&
                    (skipChance > 1 && timeIndex % 2 == 1 && skipRoll > 0 || getHoldCount() > 0) ||
                    t - lastJumpTime < timePerBeat ) {
                    steps = 0;
                    if( holds > 0 ) holds = 0;
                }                
            }
            // if( SoundEvent.DEBUG_STEPS )
            //     {
            //     makeNoteLine(lastLine, t, timeIndex % 2 == 0 ? 1 : 0, -2, allowMines);
            // } else 
            {
                makeNoteLine(lastLine, t, steps, holds, allowMines);
            }
